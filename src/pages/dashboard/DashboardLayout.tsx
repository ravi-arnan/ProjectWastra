  ...
  return (
    <div className="min-h-screen bg-surface flex">
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 w-[260px] h-screen bg-gradient-to-b from-white via-white to-surface-container-low/30 border-r border-outline-variant p-5 flex-col z-30">
        ...
        <Link
          to="/app"
          className="relative flex items-center gap-2 px-4 py-3 rounded-xl text-on-surface-variant hover:text-on-surface hover:bg-surface-container-low transition-all text-sm"
        >
      ...
      {/* ===== Mobile top bar ===== */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-background/85 backdrop-blur-xl border-b border-outline-variant/50 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-black text-primary font-headline">Dashboard</span>
        </div>
        <Link to="/app" aria-label="Tutup dashboard" className="p-2 text-on-surface-variant hover:text-on-surface">
          <Icon name="close" size="20px" />
        </Link>
      </div>

      {/* ===== Mobile bottom nav ===== */}
      <nav className="lg:hidden fixed bottom-0 w-full z-50 flex justify-around items-center px-2 pt-2 pb-5 h-[72px] bg-background/80 backdrop-blur-xl rounded-t-2xl shadow-[0_-4px_24px_rgba(31,27,23,0.06)]">
      ...
      <main className="flex-1 lg:ml-[260px]">
        ...
        <header className="hidden lg:flex sticky top-0 z-20 bg-background/80 backdrop-blur-xl items-center justify-between px-8 py-4 border-b border-outline-variant/50">
          <div>
